...
include_once("includes/logged.php");
    ...
    if($_SERVER["REQUEST_METHOD"] === "POST"){
        ...
        $price = $_POST["price"];
		$Model= $_POST["Model"];
		$Auto_Manual = $_POST["AutoManualHypered"];
		$properties = $_POST["properties"];
		// Get reference to uploaded image
		include_once("includes/addimage.php");
    try{
            $sql = "INSERT INTO `insertcar`(`carTitle`,`content`, `price`, `Model`, `AutoManualHypered`, `properties`,`image`) VALUES (?,?,?,?,?,?,?)";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$carTitle, $content, $price, $Model,$Auto_Manual,$properties,$image_name]);
		...
		<div class="container">
			<form method="POST" action="" class="m-auto" style="max-width:600px" enctype="multipart/form-data">
				...
				<div class="form-group mb-3 row"><label for="title2" class="col-md-5 col-form-label">Car Title</label>
					<div class="col-md-7"><input type="text" class="form-control form-control-lg" id="title2" name="carTitle" required></div>
				</div>
				...
				<div class="form-group mb-3 row"><label for="select-option1" class="col-md-5 col-form-label">Auto / Manual</label>
					<div class="col-md-7"><select class="form-select custom-select custom-select-lg" id="select-option1" name="AutoManualHypered">
